...
public class ResourceServerConfig  {
     ...
     * client2
     */
    private static final String RESOURCES_ID_2 = "res2";
    ...
    @Configuration
    @EnableResourceServer
    public class Client2ServerConfig extends ResourceServerConfigurerAdapter{
        @Autowired
        private TokenStore tokenStore;

        @Override
        public void configure(ResourceServerSecurityConfigurer resources) throws Exception {
            resources.tokenStore(tokenStore).resourceId(RESOURCES_ID_2).stateless(true);
        }

        @Override
        public void configure(HttpSecurity http) throws Exception {
            http.authorizeRequests().antMatchers("/client2/**").access("#oauth2.hasScope('ROLE_API')");
        }
    }
    @Configuration
    @EnableResourceServer
